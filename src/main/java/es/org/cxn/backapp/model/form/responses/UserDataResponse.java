 ...
 * <p>
 * This record is a DTO, meant to facilitate communication between the view
 * and the controller,
 * mapping all the values from the form. Each field in the record corresponds
 * to a field in the form.
 * <p>
 * The record is immutable and provides an automatic implementation of equals,
 * hashCode,
 * and toString methods. The fields include user identification, personal
 * details, and role information.
 * <p>
 * The user roles are represented as a set of {@link UserRoleName}.
 * The record also includes a nested {@link AddressResponse} record for the
 * user's address.
 * </p>
 *
...
public record UserDataResponse(
      ...
      String gender, LocalDate birthDate, String email, UserType kindMember,
      AddressResponse userAddress, Set<UserRoleName> userRoles
) {
   ...
   * values in the provided {@code UserEntity}. The user's roles are extracted
   * and stored in a set.
   * </p>
   *
  ...
  public UserDataResponse(final UserEntity user) {
    this(
          ...
          user.getSecondSurname(), user.getGender(), user.getBirthDate(),
          user.getEmail(), user.getKindMember(),
          new AddressResponse(user.getAddress()), extractUserRoles(user)
    );
  }
