...
/**
 * Represents the response form used by the controller for requesting a
 * payment sheet.
 * <p>
 * This record is a Data Transfer Object (DTO) that facilitates communication
 * between the view and the controller, providing an immutable representation
 * of the payment sheet details.
 * </p>
 *
 ...
 * @param selfVehicle the data related to the self vehicle used in the
 * payment sheet
 ...
 * @param postalCode the postal code of the payment sheet owner's address
 * @param apartmentNumber the apartment number of the payment sheet owner's
 * address
 * @param building the building of the payment sheet owner's address
 * @param street the street of the payment sheet owner's address
 * @param city the city of the payment sheet owner's address
 * @param countryName the name of the country of the payment sheet owner's
 * address
 ...
 * @author Santiago Paz
 */
public record PaymentSheetResponse(
      Integer paymentSheetIdentifier, String userName, String userFirstSurname,
      String userSecondSurname, String userDNI, String reason, String place,
      LocalDate startDate, LocalDate endDate,
      SelfVehicleDataResponse selfVehicle, FoodHousingDataResponse foodHousing,
      RegularTransportListResponse regularTransportList, String postalCode,
      String apartmentNumber, String building, String street, String city,
   ...
   * {@link PersistentPaymentSheetEntity} to the corresponding
   * fields of the {@link PaymentSheetResponse} record. It handles optional
   * values for self-vehicle and food housing data, and collects the regular
   * transport entities into a list.
   * </p>
   *
   * The method performs the following operations:
   * <ul>
   *   <li>If the {@code paymentSheetEntity} has a self-vehicle, it creates a
   *   {@link SelfVehicleDataResponse} instance with the self-vehicle data.
   *   </li>
   *   <li>If the {@code paymentSheetEntity} has food housing data, it creates
   *   a {@link FoodHousingDataResponse} instance with the food housing data.
   *   </li>
   *   <li>If the {@code paymentSheetEntity} has regular transports, it creates
   *   a {@link RegularTransportListResponse} instance from the list of regular
   *   transports.</li>
   *   <li>All other fields are directly mapped from the entity to the
   *   response.</li>
   * </ul>
   *
   * @param paymentSheetEntity the {@link PersistentPaymentSheetEntity}
   *        containing payment sheet data. Must not be {@code null}.
   * @return a new {@link PaymentSheetResponse} instance populated with data
   *         from the {@code paymentSheetEntity}.
   *
   * @throws NullPointerException if {@code paymentSheetEntity} is {@code null}.
   */
  public static PaymentSheetResponse
        fromEntity(final PersistentPaymentSheetEntity paymentSheetEntity) {
    ...
    if (paymentSheetEntity.getRegularTransports() != null
      ...
      regularTransportList = RegularTransportListResponse
            .fromEntities(paymentSheetEntity.getRegularTransports());
    } else {
      regularTransportList = null;
    }

    return new PaymentSheetResponse(
          paymentSheetEntity.getId(),
          paymentSheetEntity.getUserOwner().getName(),
          paymentSheetEntity.getUserOwner().getFirstSurname(),
          paymentSheetEntity.getUserOwner().getSecondSurname(),
          paymentSheetEntity.getUserOwner().getDni(),
          paymentSheetEntity.getReason(), paymentSheetEntity.getPlace(),
          paymentSheetEntity.getStartDate(), paymentSheetEntity.getEndDate(),
          ...
          paymentSheetEntity.getUserOwner().getAddress().getCountry()
                .getShortName(),
          paymentSheetEntity.getUserOwner().getAddress().getCountrySubdivision()
                .getName()
    );
  }
}
