 ...
 *
 * <p>These tests verify that the REST endpoints related to address
 * data retrieval work as expected.</p>
 *
 * <p>It covers scenarios such as retrieving the list of countries,
 * validating the number of subdivisions for a specific country,
 * and handling cases where a non-existing country code is requested.</p>
 *
 * @author Santiago Paz
 */
@SpringBootTest
...
class AdressControllerIntegrationTest {
   ...
   *
   * <p>This constant is used in the test methods to send GET requests to the
   * endpoint that returns the list of available countries.</p>
   */
   ...
   *
   * <p>This constant represents the number of countries that should be
   * returned by the API when the {@code GET_COUNTRIES_URL} endpoint is
   * called.</p>
   */
   ...
   *
   * <p>This constant represents the number of subdivisions that should be
   * returned by the API for Spain when its country code is used in a
   * request.</p>
   */
   ...
   *
   * <p>It sends a GET request to the endpoint that returns the list of
   * countries and verifies that the size of the list matches the
   * expected count.</p>
   *
   ...
   *
   * <p>It sends a GET request to the endpoint that returns the list of
   * subdivisions for Spain and verifies that the size of the list matches
  ...
  void testRetieveAllCountriesSubdivisionsSize() throws Exception {
    ...
    mockMvc.perform(
          get(getSpainSubdivisionsUrl).contentType(MediaType.APPLICATION_JSON)
   ...
   *
   * <p>It sends a GET request to the endpoint that returns the list of
   * subdivisions for a country, using a code that doesn't exist.
   * The test verifies that a bad request response is returned.</p>
   *
   * @throws Exception if the request fails.
   */
  @Test
  ...
  void testRetieveAllCountriesSubdivisionsNotExistingCountryBadRequest()
    ...
    final var getSpainSubdivisionsUrl =
