...
class TournamentParticipantControllerTest {
  ...
  /**
   * Tests the creation of a new tournament participant.
   * Verifies that the participant is correctly added and the response is OK.
   */
  @Test
  void testAddParticipant() throws Exception {
    ...
    mockMvc
          .perform(
                post("/api/participants").content(requestJson)
                      .contentType(MediaType.APPLICATION_JSON)
          ).andExpect(status().isCreated())
          .andExpect(
                MockMvcResultMatchers.jsonPath("$.fideId").value(FIDE_ID_JOHN)
          )
          .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("John Doe"))
          .andExpect(
                MockMvcResultMatchers.jsonPath("$.club").value("Chess Club")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$.birthDate")
                      .value(BIRTH_DATE_JOHN.toString())
          ).andExpect(
                MockMvcResultMatchers.jsonPath("$.category").value("SUB12")
          );
  }

  /**
   * Tests retrieval of all participants.
   * Verifies that the list of participants is correctly returned.
   */
  @Test
  void testGetAllParticipants() throws Exception {
    ...
    mockMvc.perform(get("/api/participants").accept(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk())
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].fideId")
                      .value(FIDE_ID_JOHN)
          )
          .andExpect(
          ...
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].club").value("Chess Club")
          )
          .andExpect(
          ...
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].category").value("SUB12")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].fideId")
                      .value(FIDE_ID_JANE)
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].name").value("Jane Smith")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].club").value("Elite Club")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].birthDate")
                      .value(BIRTH_DATE_JANE.toString())
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].category").value("SUB14")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[2].fideId")
                      .value(FIDE_ID_ALICE)
          )
          .andExpect(
          ...
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[2].club").value("Pro Club")
          )
          .andExpect(
                MockMvcResultMatchers.jsonPath("$[2].birthDate")
                      .value(BIRTH_DATE_ALICE.toString())
          ).andExpect(
                MockMvcResultMatchers.jsonPath("$[2].category").value("SUB10")
          );
  }

  /**
   * Tests the case when invalid participant data is submitted.
   * Verifies that the request is rejected with a bad request status.
   */
  @Test
  ...
  /**
   * Tests the case when no participants exist.
