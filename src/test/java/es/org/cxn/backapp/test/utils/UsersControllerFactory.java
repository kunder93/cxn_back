...
public final class UsersControllerFactory {
   ...
   */
  public static final UserRoleName DEFAULT_USER_ROLE =
   ...
   * LocalDate objects. This instance is created using a GsonBuilder with a
   * custom TypeAdapter for LocalDate to ensure proper conversion between
   * LocalDate and its JSON representation.
   *
   * The LocalDateAdapter class should implement the TypeAdapter interface
   * to define how LocalDate objects are serialized to and deserialized
   * from JSON.
   * This setup allows the Gson library to correctly process LocalDate
   * fields in JSON, ensuring compatibility with JSON APIs or data interchange
   * formats that
   * use LocalDate.
   *
   * Usage example:
