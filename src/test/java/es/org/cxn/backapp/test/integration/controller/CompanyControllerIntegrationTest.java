...
class CompanyControllerIntegrationTest {
    ...
    /**
     * JSON representation of the request to create a company with the same NIF/CIF
     * as an existing company, used to test handling of duplicate company NIF/CIF.
     */
     ...
     * This method initializes various JSON representations of request and response
     * bodies used in the integration tests for the CompanyController.
     *
     * Specifically, it prepares:
     * <ul>
     * <li>{@link CreateCompanyRequest} for a new company creation request.</li>
     * <li>{@link CreateCompanyRequest} for a request to create a company with a
     * duplicate NIF/CIF.</li>
     * <li>{@link CreateCompanyRequest} for a request to create a second
     * company.</li>
     ...
     *
     * This method is annotated with {@link BeforeAll} to ensure that it is executed
     * once before any of the test methods in the class are run, providing a
    ...
    public static void setup() {
        var companyRequest = new CreateCompanyRequest(COMPANY_NIF, COMPANY_NAME, COMPANY_ADDRESS);
        var sameNifCifCompanyRequest = new CreateCompanyRequest(COMPANY_NIF, SECOND_COMPANY_NAME,
                SECOND_COMPANY_ADDRESS);

        var secondCompanyRequest = new CreateCompanyRequest(SECOND_COMPANY_NIFCIF, SECOND_COMPANY_NAME,
                SECOND_COMPANY_ADDRESS);

        var updateCompanyRequest = new CompanyUpdateRequest(UPDATED_COMPANY_NAME, UPDATED_COMPANY_ADDRESS);

        var updateCompanyResponse = new CompanyUpdateResponse(COMPANY_NIF, UPDATED_COMPANY_NAME,
                UPDATED_COMPANY_ADDRESS);
    ...
    void testCreateCompanyCheckResponseIsOk() throws Exception {
        ...
        var requestResult = mockMvc
                .perform(post(COMPANY_URL).contentType(MediaType.APPLICATION_JSON).content(companyRequestJson))
                .andExpect(MockMvcResultMatchers.status().isCreated()).andReturn();

        var response = new CompanyResponse(COMPANY_NIF, COMPANY_NAME, COMPANY_ADDRESS);
        var gson = new Gson();
        var responseJson = gson.toJson(response);
        // Response content is the company data created.
        Assertions.assertEquals(requestResult.getResponse().getContentAsString(), responseJson,
                "create company returns company with data created");
    }

    /**
     * Using a existing Nif for create company raises a bad request response with
     * explanation message.
     *
    ...
    void testCreateCompanyExistingNifBadRequest() throws Exception {
        ...
        mockMvc.perform(post(COMPANY_URL).contentType(MediaType.APPLICATION_JSON).content(companyRequestJson))
        ...
        var responseResultContent = mockMvc
                .perform(
                        post(COMPANY_URL).contentType(MediaType.APPLICATION_JSON).content(sameNifCifCompanyRequestJson))
                .andExpect(MockMvcResultMatchers.status().isBadRequest()).andReturn().getResponse()
                .getContentAsString();

        // Response contains explanation message.
        Assertions.assertTrue(responseResultContent.contains(DefaultCompanyService.COMPANY_EXISTS),
    ...
    void testDeleteCompanyWithInvoicesNotValid() throws Exception {
        ...
        mockMvc.perform(post(COMPANY_URL).contentType(MediaType.APPLICATION_JSON).content(secondCompanyRequestJson))
                .andExpect(MockMvcResultMatchers.status().isCreated());

        // Create invoice using first company as Seller and second as Buyer.
        // Create first invoice.
        var invoiceRequest = new CreateInvoiceRequest(invoiceANumber, invoiceASeries, invoiceAPaymentDate,
                invoiceATaxExempt, invoiceASeller, invoiceABuyer, invoiceAExpeditionDate);
        ...
        mockMvc.perform(post(INVOICE_URL).contentType(MediaType.APPLICATION_JSON).content(invoiceRequestJSon))
                .andExpect(MockMvcResultMatchers.status().isCreated());

        // Deleting company is not possible, catch message.
        mockMvc.perform(delete(COMPANY_URL + "/" + COMPANY_NIF).contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isBadRequest()).andReturn().getResponse()
                .getContentAsString();
    ...
    void testDeleteNotExistingCompanyBadRequest() throws Exception {
        ...
        var deleteCompanyResult = mockMvc
                .perform(delete(COMPANY_URL + "/" + COMPANY_NIF).contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isBadRequest()).andReturn().getResponse()
                .getContentAsString();
        // Response contains not existing nifcif.
        Assertions.assertTrue(deleteCompanyResult.contains(DefaultCompanyService.COMPANY_NOT_FOUND),
    ...
    void testRemoveCompanyAndRetrieveAllCompaniesEmpty() throws Exception {
        ...
        // Get all companies
        var companiesList = mockMvc.perform(get(COMPANY_URL).contentType(MediaType.APPLICATION_JSON)
        ...
        Assertions.assertEquals(companyListJSon, companiesList, "empty companies list");
    }
    ...
    void testUpdateExistingCompany() throws Exception {
        ...
        var updateResponse = mockMvc
                .perform(put(COMPANY_URL + "/" + COMPANY_NIF).contentType(MediaType.APPLICATION_JSON)
                        .content(companyUpdateRequestJson))
                .andExpect(MockMvcResultMatchers.status().isOk()).andReturn().getResponse();

        Assertions.assertEquals(updateResponse.getContentAsString(), companyUpdateResponseJson,
    ...
    void testUpdateNotExistingCompany() throws Exception {
        ...
        mockMvc.perform(put(COMPANY_URL + "/" + COMPANY_NIF).contentType(MediaType.APPLICATION_JSON)
                .content(companyUpdateRequestJson)).andExpect(MockMvcResultMatchers.status().isBadRequest());
    }
